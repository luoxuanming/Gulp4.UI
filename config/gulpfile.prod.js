...
const checkjs = () =>
  src([config.src+'js/*.js'])
    .pipe(c$.changed(config.dist+'js', { extension: '.js' }))//仅监听有变化的js
    .pipe(c$.jshint({
      "esversion": 6
    }))
...
const less = () =>
  src(config.src+'less/*.less') //该任务针对的文件
    ...
    .pipe(dest(config.dist+'css')); //dist/css下生成main.css


//压缩css
const cleancss = () =>
  src([config.dist+'css/*.css', '!'+config.dist+'css/*.min.css'])//只选出dist/css中不含有.min.css的文件
    .pipe(c$.cleanCss())
...
const watchs = () => {
  ...
  watch(config.src+'less/*.less', series(less, cleancss))
  watch(config.src+'views/**', buildhtml)
  watch([config.src+'images/*.*', 'src/images/*/*.*'], images)
  browserSync.init({
    server: {
      baseDir: config.dist, //从dist目录中提供带有目录列表的文件
    },
    port: 8765,
...
.pipe(dest(config.dist+'libs/bootstrap/dist'));
...
.pipe(dest(config.dist+'libs/jquery'));

// exports.build = series(parallel(checkjs, copy,series(less, cleancss),series(delimages,images)), buildhtml,watchs);
module.exports = {
  checkjs,
  copy,
  less,
